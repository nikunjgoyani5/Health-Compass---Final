...
import { handleCreateSupplement as handleCreateSupplementBase } from "./health-bot.controller.js";
import { 
  // reuse existing Node handler for schedule as fallback
  // it's defined and exported in health-bot.controller.js
  // signature: (scheduleIntent, message, chatId, sessionId, authHeader, res)
  // we'll pass sessionId instead of userId
} from "./health-bot.controller.js";
...
const chatWithEnhancedHealthBot = async (req, res) => {
  try {
    ...
    // SUPER-EARLY: Medicine schedule queries must be handled before any other routing
    try {
      const hasDate = /\b\d{4}-\d{2}-\d{2}\b/.test(normalizedMessage);
      const mentionsToday = /(today|todays|today's|tonight|this\s*(morning|evening|afternoon|night))/i.test(normalizedMessage);
      const scheduleWord = /(schedule|schedu\w*|sedule)/i.test(normalizedMessage);
      const mentionsMedicine = /(medicine|medication|meds)/i.test(normalizedMessage);
      const scheduleStrong = (scheduleWord && mentionsMedicine) || (hasDate && (scheduleWord || mentionsMedicine));
      if (scheduleStrong) {
        let dateToUse = null;
        const m = normalizedMessage.match(/\b(\d{4}-\d{2}-\d{2})\b/);
        if (m) dateToUse = m[1];
        if (!dateToUse && mentionsToday) dateToUse = moment().utc().format('YYYY-MM-DD');
    ...
    // Loose detection for medicine schedule queries (allowing minor typos like "sedule", and variants like "todays")
    const isScheduleLoose = (
      /(today|todays|today's|tonight|this\s*(morning|evening|afternoon|night))/.test(normalizedMessage) &&
      /(schedule|schedu\w*|sedule)/i.test(normalizedMessage) &&
      /(medicine|medication|meds)/i.test(normalizedMessage)
    ) || /(medicine|medication|meds).*(schedule|schedu\w*|sedule)/i.test(normalizedMessage) || /(schedule|schedu\w*|sedule).*(medicine|medication|meds)/i.test(normalizedMessage);
    ...
    if (!hasHealthTerm && !isFactsheetStyle && !isPersonalizedQuery && !looksLikeFieldAnswer) {
      // Clear any creation flow so off-topic questions don't keep asking for fields
      const sessionKeyForClear = userId || chatId || req.ip || 'anonymous_session';
      if (draftCache[sessionKeyForClear]?.phase?.startsWith('create_')) {
        delete draftCache[sessionKeyForClear];
      }
      const scopeMsg =
    ...
    // 🔴 EARLY ROUTING: Medicine schedule queries BEFORE any creation fast-path
    ...
    if (isScheduleQueryEarly) {
      try {
        ...
        // Node fallback to existing handler
    ...
    // If the user explicitly asked to create a supplement, immediately route to creation flow
    ...
    if (!isScheduleLoose && (wantsCreateSupplement || mentionsSupplementExplicitly && /\bcreate\b/i.test(normalizedMessage))) {
    ...
    const isMedicineScheduleQuery = /(medicine|medication).*(schedule|schedu\w*|sedule)/i.test(normalizedMessage) || 
                                   /(schedule|schedu\w*|sedule).*(medicine|medication)/i.test(normalizedMessage) ||
                                   /(\b\d{4}-\d{2}-\d{2}\b).*(medicine|medication|schedule|schedu\w*|sedule)/i.test(normalizedMessage) ||
                                   /(medicine|medication|schedule|schedu\w*|sedule).*(\b\d{4}-\d{2}-\d{2}\b)/i.test(normalizedMessage) ||
                                   isScheduleLoose;
    ...
    if (isMedicineScheduleQuery && pythonNodeBridge.isAvailable()) {
      try {
        ...
        // Extract date from query if present, otherwise default to today
        const dateMatch = normalizedMessage.match(/\b(\d{4}-\d{2}-\d{2})\b/);
        let extractedDate = dateMatch ? dateMatch[1] : null;
        if (!extractedDate && /(today|todays|today's)/i.test(normalizedMessage)) {
          extractedDate = moment().utc().format('YYYY-MM-DD');
        }
