...
const chatWithEnhancedHealthBot = async (req, res) => {
  try {
    ...
    // BUT allow short field answers while a creation flow is in progress (e.g., "1000mg", "10 tablets", "sorry 10 tablets")
    const isCreationInProgress = !!(draftCache[userId]?.phase && (
      draftCache[userId].phase === 'create_supplement' ||
      draftCache[userId].phase === 'create_medicine' ||
      draftCache[userId].phase === 'create_vaccine' ||
      draftCache[userId].phase?.startsWith('create_')
    ));
    ...
    if (!hasHealthTerm && !isFactsheetStyle && !isPersonalizedQuery) {
      if (!(isCreationInProgress || looksLikeFieldAnswer)) {
        const scopeMsg =
      ...
      // else: we're in a creation flow or this looks like a field value; continue without blocking
    }
