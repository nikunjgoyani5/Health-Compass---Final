...
const chatWithEnhancedHealthBot = async (req, res) => {
  try {
    ...
    const wantsCreateSupplement = /(create|make|add)\s+(a\s+)?supplements?|create\s+a\s+supplement\b/i.test(normalizedMessage);
    ...
    if (!isScheduleLoose && (wantsCreateSupplement || mentionsSupplementExplicitly && /\bcreate\b/i.test(normalizedMessage))) {
      ...
      try {
        ...
        for (const [k, v] of Object.entries(prefill)) {
          if (v && !draftCache[userId].collected[k]) {
            draftCache[userId].collected[k] = v;
          }
        }
        ...
        try {
          ...
          if (detected && detected.collected) {
            draftCache[userId].collected = detected.collected;
          } else if (detected && detected.fields) {
            draftCache[userId].collected = detected.fields;
          }
        } catch (prefillErr) {
